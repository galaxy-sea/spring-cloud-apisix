 ...
 * @since 2022/10/31
 */
public class ApisixBlockingLoadBalancerClient extends BlockingLoadBalancerClient {

	public ApisixBlockingLoadBalancerClient(ReactiveLoadBalancer.Factory<ServiceInstance> loadBalancerClientFactory) {
		super(loadBalancerClientFactory);
	}

	@Override
	public URI reconstructURI(ServiceInstance serviceInstance, URI original) {
		if (serviceInstance instanceof GatewayServiceInstance) {
			return LoadBalancerUriTools.reconstructURI((GatewayServiceInstance) serviceInstance, original);
		}
		return super.reconstructURI(serviceInstance, original);
	}

	/**
	 * copy {@link org.springframework.cloud.client.loadbalancer.LoadBalancerUriTools}.
	 *
	 * @author changjin wei(魏昌进)
	 */
	private final static class LoadBalancerUriTools {
		...
		/**
		 * Modifies the URI in order to redirect the request to a service instance of
		 * choice.
		 ...
		 */
		public static URI reconstructURI(GatewayServiceInstance serviceInstance, URI original) {
		...
		private static URI doReconstructURI(GatewayServiceInstance serviceInstance, URI original) {
		...
		private static String computeScheme(URI original, GatewayServiceInstance serviceInstance) {
